...
public class Board {
	...
	private HashMap<String, Room> mapsOfRooms = new HashMap<>();
	...
	private ArrayList<Integer> assignedCharacters = new ArrayList<Integer>();
	...
	public void drawBoard() { // This method doesn't manipulate the board. Replaces outer room tiles with
		...
		for (int i = 0; i < boardRows; i++) {
			for (int j = 0; j < boardCols; j++) {
				drawBoard[3][3] = (mapsOfRooms.get("Kitchen").getWeaponChar());
				drawBoard[4][12] = (mapsOfRooms.get("BallRoom").getWeaponChar());
				drawBoard[2][21] = (mapsOfRooms.get("Conservatory").getWeaponChar());
				drawBoard[12][3] = (mapsOfRooms.get("DiningRoom").getWeaponChar());
				drawBoard[10][21] = (mapsOfRooms.get("BillardRoom").getWeaponChar());
				drawBoard[16][21] = (mapsOfRooms.get("Library").getWeaponChar());
				drawBoard[22][3] = (mapsOfRooms.get("Lounge").getWeaponChar());
				drawBoard[21][11] = (mapsOfRooms.get("Hall").getWeaponChar());
				drawBoard[22][21] = (mapsOfRooms.get("Study").getWeaponChar());
			}
			
		}
		
		//for (Room r : mapsOfRooms.values()){
		//	System.out.println(r.getWeaponChar());
	//	}
	...
	public void setupPlayer(int playerCount) { //assigns random character
		for (int k = 0; k < playerCount + 1; k++) {
			int playerCharacter =  k;
			if (!(assignedCharacters.contains(playerCharacter))) {
				assignedCharacters.add(playerCharacter);
				for (int i = 0; i < boardRows; i++) {
					for (int j = 0; j < boardCols; j++) {
						if (gameBoard[i][j] == mrsWhite && playerCharacter == 1) {
							Point pt = new Point(i, j);
							Player p1 = new Player(pt, "mrsWhite");
							listOfPlayers.add(p1);

						} else if (gameBoard[i][j] == mrGreen  && playerCharacter == 2) {
							Point pt = new Point(i, j);
							Player p2 = new Player(pt, "mrGreen");
							listOfPlayers.add(p2);
						} else if (gameBoard[i][j] == mrsPeacock  && playerCharacter == 3) {
							Point pt = new Point(i, j);
							Player p3 = new Player(pt, "mrsPeacock");
							listOfPlayers.add(p3);
						} else if (gameBoard[i][j] == colMustard  && playerCharacter == 4) {
							Point pt = new Point(i, j);
							Player p4 = new Player(pt, "colMustard");
							listOfPlayers.add(p4);

						} else if (gameBoard[i][j] == profPlum  && playerCharacter == 5) {
							Point pt = new Point(i, j);
							Player p5 = new Player(pt, "profPlum");
							listOfPlayers.add(p5);

						} else if (gameBoard[i][j] == missScarlett  && playerCharacter == 6) {
							Point pt = new Point(i, j);
							Player p6 = new Player(pt, "missScarlett");
